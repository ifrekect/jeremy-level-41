...
async function sendBirthdayPlan(data,summary){
  ...
  const response=await fetch(FORMSPREE_ENDPOINT,{method:'POST',headers:{'Content-Type':'application/json','Accept':'application/json'},body:JSON.stringify({_subject:'🎂 Jeremy Submitted His 41st Birthday Plan!',name:'Jeremy Dumas',birthday_mood:data.mood||'No answer',restaurant_choices:data.restaurants.join(', ')||'No answer',dining_out:data.dining||'No answer',home_cooked_meal:data.cooking||'No answer',activities:data.activities.join(', ')||'No answer',activity_other:data.activityOther||'No answer',gifts:data.gifts.join(', ')||'No answer',gift_notes:data.giftNotes||'No answer',perfect_birthday:data.wish||'No answer',full_birthday_summary:summary})});
